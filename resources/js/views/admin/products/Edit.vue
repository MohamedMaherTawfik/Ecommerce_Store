<template>
    <AdminLayout>
        <Transition appear name="page-fade">
            <div class="admin-page" v-if="!loading">
                <section class="admin-page-header">
                    <div class="admin-page-copy">
                        ...
                        <h2 class="admin-page-title">Update product details and attributes</h2>
                        <p class="admin-page-description">
                            Modify existing product information, manage variants, and update catalog relations.
                        </p>
                    </div>
                ...
                <section class="admin-panel">
                    <div class="admin-panel__header">
                        <div>
                            ...
                            <p class="admin-panel__meta">Update core attributes, catalog relations, and supporting
                                metadata.</p>
                        </div>
                    </div>
                    ...
                    <div class="admin-panel__body">
                        <form class="admin-form-grid" @submit.prevent="handleSubmit">
                            ...
                            <div class="admin-field">
                                ...
                                <!-- Upload new images -->
                                <input type="file" class="form-control admin-control" multiple
                                    @change="handleFileChange" accept="image/*" />

                                <!-- New selected images preview -->
                                <div v-if="form.images.length" class="mt-2 d-flex gap-2 flex-wrap">
                                    <img v-for="(img, i) in form.images" :key="i" :src="previewImage(img)"
                                        style="width:80px;height:80px;object-fit:cover" class="img-thumbnail" />
                                </div>

                                <!-- Existing images from DB -->
                                <div v-if="form.currentImages.length" class="mt-3">
                                    <small class="text-muted d-block mb-2">Current Images:</small>

                                    <div class="d-flex gap-2 flex-wrap">
                                        <div v-for="img in form.currentImages" :key="img.id" class="position-relative">
                                            <img :src="img.url" style="width:80px;height:80px;object-fit:cover"
                                                class="img-thumbnail" />
                                        </div>
                                    </div>
                                </div>
            ...
            <!-- Loading State -->
            <div v-else class="admin-page d-flex justify-content-center align-items-center min-vh-100">
...
// --- Helper: Normalize API data for dynamic inputs ---
// Handles cases where API returns: ["S", "M"] OR [{id:1, name:"S"}] OR [{value:"S"}]
const normalizeArrayData = (data, valueKey = 'name') => {
    if (!Array.isArray(data)) return [];

    return data.map((item, index) => {
        // If it's a simple string
        if (typeof item === 'string') {
            return { id: ++sizeIdCounter, value: item };
        }
        // If it's an object, try to find the value property
        const val = item[valueKey] || item.value || item.name || '';
        return { id: item.id || ++sizeIdCounter, value: val };
    });
};
...
const fetchProduct = async () => {
    ...
    try {
        ...
        const product = getRecord(response);

        form.currentImages = product.images?.map(img => ({
            id: img.id,
        ...
        form.categories_id = product.categories_id ?? "";
        form.brands_id = product.brands_id ?? "";
        form.currentImage = product.image ?? "";
        form.is_active = Boolean(product.is_active);
        form.is_featured = Boolean(product.is_featured);
        form.return_policy = product.return_policy ?? "";
        ...
        form.og_title = product.og_title ?? "";
        form.og_description = product.og_description ?? "";
        form.og_image = product.og_image ?? null;
        form.canonical_url = product.canonical_url ?? "";
        form.sku = product.sku ?? "";
        form.tax = product.tax ?? "";
        form.quantity = product.stocks?.quantity ?? 0;

        // Map Dynamic Arrays (Sizes & Colors)
        // Assuming API returns relations 'sizes' and 'colors' based on your Controller
        form.sizes = normalizeArrayData(product.sizes, 'size');
        form.colors = normalizeArrayData(product.colors, 'color');

        // Update counters to avoid ID collisions when adding new items
        if (form.sizes.length > 0) {
            sizeIdCounter = Math.max(...form.sizes.map(s => s.id)) + 1;
        }
        if (form.colors.length > 0) {
            colorIdCounter = Math.max(...form.colors.map(c => c.id)) + 1;
        }
...
const buildPayload = () => {
    ...
    // Image (only if new one selected)
...
const handleSubmit = async () => {
    ...
    try {
        await productService.updateProduct(route.params.id, buildPayload());
        await router.push("/admin/products");
    } catch (error) {
        console.error(`Failed to update product ${route.params.id}:`, error);
...
// --- Auto-generation Logic (Watchers) ---
...
watch(
    ...
    (newName) => {
        if (!newName) return;
        // Only auto-generate if the user hasn't manually edited these fields
        if (!isSkuEdited.value) form.sku = generateSKU(newName);
        if (!isMetaTitleEdited.value) form.meta_title = newName;
        if (!isMetaDescriptionEdited.value) form.meta_description = generateMetaDescription(newName);
    }
);
onMounted(async () => {
    await fetchOptions();
...
<style scoped>
/* --- Base Styles (Mirroring Create Page) --- */
.admin-page {
    padding: 1.5rem;
}
